function [Xproj, Yproj, Kx, Ky, cor, XunpairedProj,YunpairedProj, OSEoutputs] = ...
    NCCA_KernelsAsInputs(Kx, Ky, d, Xunpaired, Yunpaired, params)

%%%%%
% This function is based on the function "NCCA.m". 
% Unlike NCCA:
% - this function gets the kernels as inputs
% - the SVD decomposition (in line 170 in NCCA.m) is computed using Matlab's SVD function and not approximated using randpca_AB.m.
% - a bug was fixed: "~isfield('params','hx')" -> "~isfield(params,'hx')"
% All this changes were made to enable fair comparison with NCCA.
%%%%%
...
N = size(Kx,1); % Number of paired training points
...
% % Ky = Dy;
% 
% % figure; stem(sort(real(eig(full(Kx))), 'descend'));
% 
...
% tic; [U,D,V] = randpca_AB(Dx, Dy, d+1, randSVDiters, randSVDblock);
tic; [U,D,V] = svd(full(Dx* Dy));
U=U(:,1:d+1); D=D(1:d+1,1:d+1); V=V(:,1:d+1);
